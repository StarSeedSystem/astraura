...
class ProjectsManager:
    ...
    def auto_assign_to_project(self, item_id: str, item_type: str, title: str, tags: List[str]) -> str:
        """
        Analiza inteligentemente a qué proyecto automático enlazar el elemento.
        Si no hay coincidencia clara, crea un nuevo proyecto automático.
        """
        ...
        # Si no hay match, crear uno nuevo basado en el tag principal o el título
        main_tag = tags[0] if tags else "General"
        new_proj_name = f"Proyecto Automático: {main_tag.capitalize()}"
        new_proj = self.create_project(
            name=new_proj_name,
            description=f"Proyecto generado inteligentemente para agrupar creaciones sobre {main_tag}.",
